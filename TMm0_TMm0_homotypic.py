...
if __name__ == "__main__":
    ...
    savepath = r"D:\PhD\HOMmix\HOMmix_analytical\analysis\homotypic_monopoles"
    m_max, n_max, p_max = 0, 3, 3
    voxel_res = 151
    f_010 = 1.3e9
    c_sol = 299792458.0
    create_data = True
    create_fields = True
    if create_data:
        data_dict = assemble_all_data_dict(m_max, n_max, p_max,
                                                    frequency_010=f_010,
                                                    LF_start=0.7,
                                                    LF_stop=1.3,
                                                    param_sweep_resolution=1000,
        ...
        hamm.pickle_save(data_dict, f"{datapath}\\TMm0_TMm0_data_dict_{voxel_res}x{voxel_res}x{voxel_res}.pkl")

    else:
        data_dict = hamm.pickle_load(f"{datapath}\\TMm0_TMm0_data_dict_{voxel_res}x{voxel_res}x{voxel_res}.pkl")
