    ...
    *,
    ...
    for result in filtered:
        ...
        for metric_index, metric_key in enumerate(metric_keys):
            ...
            row_ending = r" \\*" if metric_index == 0 else r" \\"
...
""".strip()
    ...
    for table_index, table_text in enumerate(table_blocks):
        # \clearpage flushes any preceding table* float and guarantees that
        # the next table cannot begin until the current one has completely
        # finished, even when this leaves white space on the previous page.
