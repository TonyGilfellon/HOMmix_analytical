...
def heterotypic_table(
    ...
    *,
    ...
    metric_keys = heterotypic_metric_keys(
        pair_type
    )
    pair_name = pair_type.replace(
        "_",
        "--",
    )

    body: list[str] = []
    previous_crossing: tuple[str, str, float] | None = None
    ...
    return rf"""
\begin{{table*}}[htbp]
\caption{{Aggregated heterotypic {pair_name} Taylor/Hessian results. Only the uppercase, length-normalised metrics relevant to the two parent families are reported.}}
\label{{tab:aggregate_heterotypic_{label_pair}}}
\begin{{ruledtabular}}
