...
# -----------------------------------------------------------------------------
# Self-contained homotypic quadrupole crossing workflow
# -----------------------------------------------------------------------------
...
def focusing_summary_line(name: str, result: dict) -> str:
    """Compact one-line Brett quadrupole-strength summary."""
    return (
        f"  {name:5s}: K_Q={result['K_Q_V_per_pC_per_m3']:.6e} V/pC/m^3; "
        f"rho={result.get('sample_radius_m', float('nan')):.6e} m; "
        f"b2={result.get('brett_b2_complex', complex(float('nan'), float('nan'))):.6e}; "
        f"a2={result.get('brett_a2_complex', complex(float('nan'), float('nan'))):.6e}"
    )
...
def write_crossing_quadrupole_summary(out_file: str | Path, *, key: str, mode_i: str, mode_j: str, crossing: dict, focusing: dict) -> None:
    ...
    lines.append("  U_CST_J = 0.5 eps0 integral |E|^2 dV")
    lines.append("  z convention = z in [0,L], centre_z=False")
    lines.append("  method = Brett azimuthal RF-multipole extraction")
    lines.append("  Vz(phi) = int Ez(rho,phi,z) exp(+i omega z/beta c) dz")
    lines.append("  Vz ~= rho^2 [b2 cos(2 phi) + a2 sin(2 phi)]")
    lines.append("  K_Q = 2*(2c/omega)*sqrt(|b2|^2+|a2|^2)/sqrt(4 U_CST)/length_m * 1e-12 [V/pC/m^3]")
    lines.append("")
    for name in ("E1", "E2", "plus", "minus"):
        r = focusing[name]
        lines.append(f"{name}:")
        lines.append(f"  U_CST_J                       = {r['U_CST_J']:.12e}")
        lines.append(f"  sample_radius_m               = {r['sample_radius_m']:.12e}")
        lines.append(f"  brett_b2                      = {r['brett_b2_complex'].real:.12e} {r['brett_b2_complex'].imag:+.12e}j")
        lines.append(f"  brett_a2                      = {r['brett_a2_complex'].real:.12e} {r['brett_a2_complex'].imag:+.12e}j")
        lines.append(f"  K_grad_raw_Brett              = {r['K_grad_raw_Brett_V_per_C_per_m_per_m']:.12e}")
        lines.append(f"  K_Q_raw_hessian_equiv         = {r['K_Q_raw_hessian_equiv_V_per_C_per_m_per_m']:.12e}")
        lines.append(f"  K_grad_U_CST_norm             = {r['K_grad_U_CST_norm']:.12e}")
        lines.append(f"  K_Q_U_CST_norm                = {r['K_Q_U_CST_norm']:.12e}")
...
def analyse_crossing(
    ...
    *,
    ...
    rot_i = align_field_to_vertical_plane(
        raw_i,
        out_plot=str(out_dir / f"TM{mode_i}_theta_r_rotation.png"),
        label=f"TM{mode_i}",
    )
    rot_j = align_field_to_vertical_plane(
        raw_j,
    ...
    save_field_data_npz(field_data, str(out_dir / "field_data.npz"))
    ...
    for name, (Ex, Ey, Ez, freq, lf) in focus_jobs.items():
        focusing[name] = quadrupole_focusing_from_Ez_field(
            Ez,
            f_010=f_010,
            f_mnp=freq,
            l_factor=lf,
    ...
    analysis = {
        ...
        "focusing_units": "reported scalar K_Q is from Brett azimuthal RF-multipole extraction in V/pC/m^3",
        "focusing_sign_convention": "Brett homotypic quadrupole analysis reports scalar K_Q magnitude only; no signed Kxx/Kxy/Kyy.",
        "focusing": focusing,
        "files": {
    ...
    pickle_save(analysis, out_dir / "crossing_analysis.pkl")
...
def main():
    ...
    if create_data:
        ...
        pickle_save(data_dict, data_file)
    else:
        data_dict = pickle_load(data_file)
    ...
    pickle_save(crossing_results, savepath / "crossing_results.pkl")
    ...
    pickle_save(analyses, savepath / "all_crossing_analyses.pkl")
