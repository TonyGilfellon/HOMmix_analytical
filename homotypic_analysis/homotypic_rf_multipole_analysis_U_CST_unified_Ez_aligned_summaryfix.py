...
def align_field_to_vertical_plane(
    ...
    *,
    ...
    """Align the longitudinal field with the vertical longitudinal plane.

    The RF multipole coefficients are obtained from ``Ez`` and the complex
    longitudinal voltage derived from it. The production orientation criterion
    is therefore the global maximum of ``abs(Ez)``, rather than the total field
    magnitude ``|E|``.

    After rotation, at least one global ``abs(Ez)`` maximum must be present in

        ``abs(Ez_rotated)[mid_x, :, :]``.
    """
    ...
    Ez_abs_before = np.abs(np.nan_to_num(Ez, nan=0.0))
    angle_deg, peak_before = rotation_angle_to_vertical_plane(
        Ez_abs_before,
        x_m,
        y_m,
    )
    ...
    Ez_after = np.asarray(rotated['Ez'])
    Ez_abs_after = np.abs(Ez_after)
    Eabs_after = np.asarray(rotated['|E|'])
    mid_x = nx // 2

    first_global_Ez_peak = tuple(
        int(v)
        for v in np.unravel_index(
            np.nanargmax(Ez_abs_after),
            Ez_abs_after.shape,
        )
    )
    global_Ez_max = float(np.nanmax(Ez_abs_after))
    vertical_Ez_plane = Ez_abs_after[mid_x, :, :]
    vertical_Ez_max = float(np.nanmax(vertical_Ez_plane))

    if not np.isclose(
        vertical_Ez_max,
        global_Ez_max,
        rtol=1.0e-8,
        atol=max(1.0e-14, 1.0e-10 * max(global_Ez_max, 1.0)),
    ):
        raise RuntimeError(
            f'{label}: rotation did not place a global |Ez| maximum in '
            f'abs(Ez)[{mid_x}, :, :]; global={global_Ez_max:.12e}, '
            f'vertical-plane={vertical_Ez_max:.12e}, '
            f'first_global_Ez_peak={first_global_Ez_peak}, '
    ...
    iy_plane, iz_plane = (
        int(v)
        for v in np.unravel_index(
            np.nanargmax(vertical_Ez_plane),
            vertical_Ez_plane.shape,
        )
    )
    peak_after = (int(mid_x), iy_plane, iz_plane)

    global_Eabs_max = float(np.nanmax(Eabs_after))
    vertical_Eabs_max = float(np.nanmax(Eabs_after[mid_x, :, :]))
    Eabs_vertical_fraction = (
        vertical_Eabs_max / global_Eabs_max
        if global_Eabs_max > 0.0
        else float('nan')
    )
    ...
    diagnostics = {
        ...
        'alignment_target': 'abs(Ez)',
        'rotation_angle_deg': float(angle_deg),
        'peak_before_xyz': peak_before,
        'peak_after_xyz': peak_after,
        'first_global_Ez_peak_after_xyz': first_global_Ez_peak,
        'mid_x_index': int(mid_x),
        'global_max_abs_Ez_after': global_Ez_max,
        'vertical_plane_max_abs_Ez_after': vertical_Ez_max,
        'vertical_plane_contains_global_abs_Ez_max': True,
        'global_max_Eabs_after': global_Eabs_max,
        'vertical_plane_max_Eabs_after': vertical_Eabs_max,
        'vertical_plane_Eabs_fraction': Eabs_vertical_fraction,
    }

    print(
        f'{label}: Ez-aligned by {angle_deg:.6f} deg; '
        f'|Ez| peak {peak_before} -> vertical-plane peak {peak_after}; '
        f'first global |Ez| argmax={first_global_Ez_peak}; '
        f'abs(Ez)[{mid_x}, :, :] contains a global |Ez| maximum; '
        f'|E| vertical/global={Eabs_vertical_fraction:.6f}.'
    )
...
def get_or_create_field_data(
    ...
    *,
    ...
    if not recreate and int(family_m) in (1, 2):
        ...
        alignment_target = existing.get('vertical_plane_alignment_target')
        alignment_target_text = (
            str(np.asarray(alignment_target).item())
            if alignment_target is not None
            else ''
        )

        if (
            rotation_flag is None
            or not bool(np.asarray(rotation_flag).item())
            or alignment_target_text != 'abs(Ez)'
        ):
            print(
                f'{filename}: old or non-Ez-aligned field data detected; '
                f'regenerating.'
            )
            recreate = True
        else:
    ...
    if recreate:
        ...
        data['vertical_plane_alignment_target'] = np.asarray('abs(Ez)')
        ...
        for prefix, diagnostics in (
            ...
            data[f'{prefix}_global_max_abs_Ez_after'] = np.asarray(
                diagnostics['global_max_abs_Ez_after'], dtype=float
            )
            data[f'{prefix}_vertical_plane_max_abs_Ez_after'] = np.asarray(
                diagnostics['vertical_plane_max_abs_Ez_after'], dtype=float
            )
    ...
    for parent_name in ("E1", "E2"):
        ...
        if diag:
            lines.extend([
                ...
                f"  {parent_name} alignment_target = {diag.get('alignment_target', 'legacy |E|')}",
            f"  {parent_name} vertical_plane_contains_global_abs_Ez_max = "
            f"{diag.get('vertical_plane_contains_global_abs_Ez_max', diag.get('vertical_plane_contains_global_max', False))}",
            ])
    lines.append("")
