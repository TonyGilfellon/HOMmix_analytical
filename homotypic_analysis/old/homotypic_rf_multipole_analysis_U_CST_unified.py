...
def plot_field_slices_combined(field_data: dict[str, np.ndarray], out_dir: Path, title: str) -> None:
    """Retain the established 4x4 [E1,E2,E+,E-] slice plots."""
    out_dir.mkdir(parents=True, exist_ok=True)
    slice_specs = {
        "iris_1": lambda a: np.asarray(a)[:, :, 0].T,
        "iris_2": lambda a: np.asarray(a)[:, :, -1].T,
        "transverse_mid": lambda a: np.asarray(a)[:, :, np.asarray(a).shape[2] // 2].T,
        "longitudinal_mid": lambda a: np.asarray(a)[np.asarray(a).shape[0] // 2, :, :],
    }
    rows = [
    ...
    for name, slicer in slice_specs.items():
        fig, axes = plt.subplots(4, 4, figsize=(14, 10), constrained_layout=True)
        fig.suptitle(f"{title}: {name}")
        parent_ez_ref = max(
            float(np.nanmax(np.abs(slicer(field_data["E1_Ez"])))),
            float(np.nanmax(np.abs(slicer(field_data["E2_Ez"])))),
            1e-300,
        )
        parent_abs_ref = max(
            float(np.nanmax(np.abs(slicer(field_data["abs_E1"])))),
            float(np.nanmax(np.abs(slicer(field_data["abs_E2"])))),
            1e-300,
        )
        for r, keys in enumerate(rows):
            images = [slicer(field_data[k]) for k in keys]
            ref = parent_abs_ref if r == 3 else parent_ez_ref
            scaled = [a / ref for a in images]
            vmax = max(1.0, max(float(np.nanmax(np.abs(a))) for a in scaled))
            for c, a in enumerate(scaled):
                ax = axes[r, c]
                if r == 3:
                    im = ax.imshow(a, origin="lower", aspect="auto", vmin=0.0, vmax=vmax, cmap="viridis")
                else:
                    im = ax.imshow(a, origin="lower", aspect="auto", vmin=-vmax, vmax=vmax, cmap="RdBu_r")
                if r == 0:
                ...
                ax.set_xticks([])
                ax.set_yticks([])
            fig.colorbar(im, ax=axes[r, :], shrink=0.75)
        fig.savefig(out_dir / f"{name}.png", dpi=250)
        plt.close(fig)
