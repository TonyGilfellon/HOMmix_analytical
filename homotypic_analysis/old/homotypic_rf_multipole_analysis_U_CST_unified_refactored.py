...
def align_field_to_vertical_plane(
    ...
    *,
    ...
    # Match the successful earlier implementation. One-pixel tolerance allows
    # for interpolation ties on an odd Cartesian grid, while the plane maximum
    # check confirms that the vertical section contains the global peak value.
    if abs(peak_after[0] - mid_x) > 1:
        raise RuntimeError(
            f'{label}: rotation failed; peak_after={peak_after}, '
            f'expected x index {mid_x}, angle={angle_deg:.12g} deg.'
        )

    global_max = float(np.nanmax(Eabs_after))
    vertical_plane_max = float(np.nanmax(Eabs_after[mid_x, :, :]))
    if not np.isclose(
        vertical_plane_max,
        global_max,
        rtol=1.0e-8,
        ...
        raise RuntimeError(
            f'{label}: rotation did not place the global |E| maximum in '
            f'Eabs[{mid_x}, :, :]; global={global_max:.12e}, '
            f'vertical-plane={vertical_plane_max:.12e}, '
            f'peak_after={peak_after}, angle={angle_deg:.12g} deg.'
        )
...
def main(cfg: RunConfig = RunConfig()) -> dict[int, dict[str, Any]]:
    ...
    all_results = {2: analyse_family(cfg, 2)}  # just quadrupole
