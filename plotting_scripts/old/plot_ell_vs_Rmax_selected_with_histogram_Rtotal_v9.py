...
def _collect_rtotal_and_ell_populations(
    *,
    ...
    """Collect cut-off-filtered R_total and ell values for both populations."""
    ...
    for group_results, metric_key, _marker, label_color in combined_groups:
        ...
        target_ell = homotypic_ell if is_homotypic else heterotypic_ell
        ...
        for result in group_results:
            ...
            if point is None:
                continue
            ell, r_total = point
            target_ell.append(ell)
            target_rtotal.append(r_total)

    return homotypic_rtotal, heterotypic_rtotal, homotypic_ell, heterotypic_ell
