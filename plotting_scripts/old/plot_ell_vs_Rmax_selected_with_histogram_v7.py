...
def _write_rmax_histogram(
    *,
    ...
    homo_mean = np.mean(homotypic_rmax)
    homo_std = np.std(homotypic_rmax)

    het_mean = np.mean(heterotypic_rmax)
    het_std = np.std(heterotypic_rmax)
...
def write_selected_ell_vs_rmax_plots(
    *,
    ...
    """Write only the combined log plot, annotated zoom, and R_max histogram."""
