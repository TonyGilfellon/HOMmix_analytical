...
def _statistics_row_plot_label(row: dict[str, Any]) -> str:
    distribution = str(row["distribution"]).replace("$", "")
    distribution = distribution.replace(r"R_{K_{\parallel}}", r"R_parallel")
    distribution = distribution.replace(r"R_{K_{\perp}}", r"R_perp")
    distribution = distribution.replace(r"R_{K_Q}", r"R_Q")
    return f"{row['crossing_type']} {distribution}"
...
def _write_rk_mean_vs_standard_deviation_plot(
    *,
    ...
    """Plot crossing-type mean R_K against sample standard deviation."""
    valid_rows = [r for r in rows if math.isfinite(r["mean"]) and math.isfinite(r["sample_sd"])]
    if not valid_rows:
        raise ValueError("No finite mean/sample-standard-deviation pairs are available.")
    fig, ax = plt.subplots(figsize=PRAB_FIGSIZE)
    for row in valid_rows:
        is_homotypic = row["crossing_type"] in {"M-M", "D-D", "Q-Q"}
        colour = HOMOTYPIC_COLOUR if is_homotypic else HETEROTYPIC_COLOUR
        marker = "o" if is_homotypic else "s"
        ax.plot(row["mean"], row["sample_sd"], linestyle="none", marker=marker,
                markersize=DEFAULT_MARKER_SIZE, markerfacecolor="none",
                markeredgecolor=colour, markeredgewidth=DEFAULT_MARKER_EDGE_WIDTH,
                zorder=3)
        ax.annotate(_statistics_row_plot_label(row),
                    (row["mean"], row["sample_sd"]), xytext=(3,3),
                    textcoords="offset points", fontsize=5.5)
    ax.set_xlabel(r"Sample mean, $\bar{R}_K$", fontsize=AXIS_LABEL_FONTSIZE)
    ax.set_ylabel(r"Sample standard deviation, $s_{R_K}$", fontsize=AXIS_LABEL_FONTSIZE)
    ax.grid(True, which="both", alpha=0.3)
    ax.plot([], [], linestyle="none", marker="o", markersize=DEFAULT_MARKER_SIZE,
            markerfacecolor="none", markeredgecolor=HOMOTYPIC_COLOUR,
            markeredgewidth=DEFAULT_MARKER_EDGE_WIDTH, label="Homotypic")
    ax.plot([], [], linestyle="none", marker="s", markersize=DEFAULT_MARKER_SIZE,
            markerfacecolor="none", markeredgecolor=HETEROTYPIC_COLOUR,
            markeredgewidth=DEFAULT_MARKER_EDGE_WIDTH, label="Heterotypic")
    legend = ax.legend(loc="best", frameon=True, framealpha=1.0,
                       facecolor="white", edgecolor="black",
                       fontsize=LEGEND_FONTSIZE)
    _apply_fine_plot_styling(ax)
    _style_legend_frame(legend)
    fig.tight_layout()
    output_file = output_root / RK_HISTOGRAM_SUBDIRECTORY / "crossing_type_RK_mean_vs_sample_standard_deviation.png"
    output_file.parent.mkdir(parents=True, exist_ok=True)
    fig.savefig(output_file, bbox_inches="tight", dpi=dpi, format="png")
    plt.close(fig)
    print(f"Wrote: {output_file}")
    print(f"  plotted distributions: {len(valid_rows)}")
    return output_file
