...
RK_BOX_PLOT_SCALE = "log"  # "linear" or "log"
...
# Three-panel histogram typography.
COMPOSITE_AXIS_LABEL_FONTSIZE = 13.0
COMPOSITE_TICK_FONTSIZE = 11.0
COMPOSITE_LEGEND_FONTSIZE = 9.0
COMPOSITE_INSET_FONTSIZE = 13.0

# Overall-plus-zoom two-panel R_total figure.
RTOTAL_TWO_PANEL_FIGSIZE = (10.2, 4.2)
RTOTAL_TWO_PANEL_AXIS_LABEL_FONTSIZE = 14.0
RTOTAL_TWO_PANEL_TICK_FONTSIZE = 11.0
RTOTAL_TWO_PANEL_LEGEND_FONTSIZE = 8.5
RTOTAL_TWO_PANEL_ZOOM_LABEL_FONTSIZE = 10.0
RTOTAL_TWO_PANEL_MARKER_SCALE = 1.15
...
def _rk_legend_label(base_label: str, metric_key: str) -> str:
    """Return the compact R_K notation used only by the two-panel figure."""
    symbol = _metric_legend_symbol(metric_key)
    return rf"{base_label} $R_{{{symbol}}}$"


def _plot_rtotal_groups_on_axis(
    ax: Any,
    *,
    combined_groups: list[tuple[list[dict[str, Any]], str, str, str]],
    cutoffs: dict[str, float],
    ...
    """Plot retained R_K groups on a supplied axis."""
    count = 0

    for group_results, metric_key, marker, label_color in combined_groups:
        label, colour = label_color.split("|||", 1)
        ...
        for result in group_results:
            ...
            if zoom_bounds is not None:
                ell_min, ell_max, r_min, r_max = zoom_bounds
                if not (
                    ell_min <= ell <= ell_max
                    and r_min <= r_value <= r_max
                ):
                    continue

            points.append((ell, r_value))
        ...
        _plot_hollow_markers(
            ...
            y_values,
            marker=marker,
            label=_rk_legend_label(label, metric_key),
            color=colour,
            markersize=DEFAULT_MARKER_SIZE * marker_scale,
            markeredgewidth=DEFAULT_MARKER_EDGE_WIDTH * marker_scale,
        )
        count += len(points)
...
def _write_combined_rtotal_overall_and_zoom_log_plot(
    *,
    combined_groups: list[tuple[list[dict[str, Any]], str, str, str]],
    cutoffs: dict[str, float],
    output_root: Path,
    ...
    """Write the full logarithmic plot at left and its logarithmic zoom at right."""

    # Left: overall population. Right: zoom.
    fig, (ax_overall, ax_zoom) = plt.subplots(
        1,
        2,
        figsize=RTOTAL_TWO_PANEL_FIGSIZE,
        gridspec_kw={"width_ratios": [1.18, 1.0]},
    )

    overall_count = _plot_rtotal_groups_on_axis(
        ax_overall,
        combined_groups=combined_groups,
        cutoffs=cutoffs,
        marker_scale=RTOTAL_TWO_PANEL_MARKER_SCALE,
    )
    zoom_count = _plot_rtotal_groups_on_axis(
        ax_zoom,
        combined_groups=combined_groups,
        cutoffs=cutoffs,
        zoom_bounds=(ell_min, ell_max, rtotal_min, rtotal_max),
        marker_scale=RTOTAL_TWO_PANEL_MARKER_SCALE,
    )

    for ax in (ax_zoom, ax_overall):
        ax.set_yscale("log")
        ax.axhline(
            1.0,
            color=REFERENCE_COLOUR,
        ...
        ax.set_xlabel(
            r"$\ell$",
            fontsize=RTOTAL_TWO_PANEL_AXIS_LABEL_FONTSIZE,
        )
        ax.grid(True, which="both", alpha=0.3)
        _apply_fine_plot_styling(ax)
        ax.tick_params(
            axis="both",
            labelsize=RTOTAL_TWO_PANEL_TICK_FONTSIZE,
        )
    ...
    ax_zoom.set_xlim(ell_min, ell_max)
    ax_zoom.set_ylim(rtotal_min, rtotal_max)

    # Explicit integer tick labels for the left-hand zoom panel.
    ax_zoom.set_yticks([1, 2, 3, 4, 5, 6])
    ax_zoom.set_yticklabels(
        ["1", "2", "3", "4", "5", "6"],
        fontsize=RTOTAL_TWO_PANEL_TICK_FONTSIZE,
    )
    ...
    ax_overall.add_patch(zoom_rectangle)
    # Connect the right-hand corners of the rectangle in the overall panel
    # to the left-hand bounds of the right zoom panel.
    for source_y, target_y in (
        (rtotal_min, rtotal_min),
        (rtotal_max, rtotal_max),
    ):
        connector = ConnectionPatch(
            xyA=(ell_max, source_y),
            coordsA=ax_overall.transData,
            xyB=(ell_min, target_y),
    ...
    # Use one enlarged legend in the left-hand overall panel.
    ...
    legend = ax_overall.legend(
        ...
        facecolor="white",
        edgecolor="black",
        fontsize=RTOTAL_TWO_PANEL_LEGEND_FONTSIZE,
        markerscale=1.0,
    )
    _style_legend_frame(legend)
    legend.set_zorder(30)

    # The connectors are drawn above axes content but below the opaque
    # legend, using connector zorder=20 and legend zorder=30.
    fig.subplots_adjust(
        left=0.085,
        right=0.985,
        bottom=0.16,
    ...
    output_file = (
        output_root
        / "homotypic_and_heterotypic_ell_vs_RK_overall_and_zoom_log.png"
    )
    output_file.parent.mkdir(parents=True, exist_ok=True)
    fig.savefig(
        output_file,
    ...
    print(f"Wrote: {output_file}")
    print(
        f"  overall points={overall_count}; zoom points={zoom_count}; "
        f"zoom ell=[{ell_min}, {ell_max}]; "
        f"zoom R_K=[{rtotal_min}, {rtotal_max}]"
    )
    return output_file
...
def _write_ell_and_rtotal_histogram_two_panel(
    *,
    homotypic_rtotal: list[float],
    heterotypic_rtotal: list[float],
    homotypic_ell: list[float],
    ...
    """
    Write a two-panel histogram figure containing:

      left:  linear ell histogram;
      right: logarithmic R_total histogram.

    The binning and visual styling match the corresponding standalone plots.
    """
    all_ell = [
        value
        for value in homotypic_ell + heterotypic_ell
        if math.isfinite(value)
    ]
    if not all_ell:
        raise ValueError("No finite ell values are available for histogramming.")
    if ell_n_bins < 1:
        raise ValueError("The ell histogram bin count must be at least 1.")
    if rtotal_n_bins < 1:
        raise ValueError("The R_total histogram bin count must be at least 1.")

    ell_minimum = min(all_ell)
    ell_maximum = max(all_ell)
    if ell_minimum == ell_maximum:
        ell_minimum -= 0.05
        ell_maximum += 0.05

    ell_bins = np.linspace(
        ell_minimum,
        ell_maximum,
        ell_n_bins + 1,
    )
    rtotal_bins = _shared_histogram_bins(
        homotypic_rtotal + heterotypic_rtotal,
        scale="log",
        n_bins=rtotal_n_bins,
    )

    fig, (ax_ell, ax_rtotal) = plt.subplots(
        1,
        2,
        figsize=(10.2, 4.2),
    )

    # Left panel: ell histogram.
    ax_ell.hist(
        heterotypic_ell,
        bins=ell_bins,
        histtype="stepfilled",
    ...
    ax_ell.set_xlabel(r"$\ell$", fontsize=AXIS_LABEL_FONTSIZE)
    ax_ell.set_ylabel("Crossing count", fontsize=AXIS_LABEL_FONTSIZE)
    ax_ell.grid(True, which="both", alpha=0.3)
    ell_legend = ax_ell.legend(
        loc="upper left",
        frameon=True,
        framealpha=1.0,
        facecolor="white",
    ...
    _apply_fine_plot_styling(ax_ell)
    _style_legend_frame(ell_legend)

    # Right panel: logarithmic R_total histogram.
    ax_rtotal.hist(
        heterotypic_rtotal,
        bins=rtotal_bins,
        histtype="stepfilled",
    ...
    ax_rtotal.set_xlabel(
        r"$R_{\mathrm{total}}$",
        fontsize=AXIS_LABEL_FONTSIZE,
    )
    ax_rtotal.set_ylabel("Crossing count", fontsize=AXIS_LABEL_FONTSIZE)
    ax_rtotal.grid(True, which="both", alpha=0.3)
    rtotal_legend = ax_rtotal.legend(
        loc="upper right",
        frameon=True,
        framealpha=1.0,
        facecolor="white",
    ...
    _apply_fine_plot_styling(ax_rtotal)
    _style_legend_frame(rtotal_legend)

    fig.tight_layout()

    output_file = (
        output_root
        / "homotypic_and_heterotypic_ell_linear_and_Rtotal_log_histograms_two_panel.png"
    )
    output_file.parent.mkdir(parents=True, exist_ok=True)
    fig.savefig(
        output_file,
    ...
    print(f"Wrote: {output_file}")
    print(
        "  two-panel histograms: "
        f"ell homotypic={len(homotypic_ell)}, "
        f"ell heterotypic={len(heterotypic_ell)}, "
        f"R_total homotypic={len(homotypic_rtotal)}, "
        f"R_total heterotypic={len(heterotypic_rtotal)}"
    )
    return output_file
...
def _write_homotypic_rk_histogram_composite(
    *,
    homotypic: dict[int, list[float]],
    bins: np.ndarray,
    output_root: Path,
    ...
    """Write a three-panel M-M, D-D and Q-Q homotypic R_K histogram."""
    ...
    for ax, family_m in zip(axes, (0, 1, 2)):
        info = HOMOTYPIC_PLOT_INFO[family_m]
        ...
        ax.set_xlabel(
            x_labels[family_m],
            fontsize=COMPOSITE_AXIS_LABEL_FONTSIZE,
        )
    ...
    for ax in axes:
        ax.tick_params(
            axis="both",
            labelsize=COMPOSITE_TICK_FONTSIZE,
        )

    output_file = (
        output_root
        / RK_HISTOGRAM_SUBDIRECTORY
        / f"homotypic_RK_histograms_3_panel_{rtotal_scale}.png"
    )
    output_file.parent.mkdir(parents=True, exist_ok=True)
    fig.savefig(
        output_file,
...
def _write_heterotypic_rk_histogram_composite(
    *,
    heterotypic: dict[str, dict[str, list[float]]],
    bins: np.ndarray,
    output_root: Path,
    ...
    """Write a three-panel M-D, M-Q and D-Q heterotypic R_K histogram.

    Each panel retains the existing convention:
      * first applicable metric: blue outline;
      * second applicable metric: orange outline;
      * combined distribution: translucent black fill.
    """
    ...
    for ax, pair_type in zip(axes, pair_types):
        ...
        legend = ax.legend(
            ...
            facecolor="white",
            edgecolor="black",
            fontsize=composite_legend_fontsize,
            markerscale=LEGEND_MARKERSCALE,
        )
        _style_legend_frame(legend)
    ...
    output_file = (
        output_root
        / RK_HISTOGRAM_SUBDIRECTORY
        / f"heterotypic_RK_histograms_3_panel_{rtotal_scale}.png"
    )
    output_file.parent.mkdir(parents=True, exist_ok=True)
    fig.savefig(
        output_file,
...
def _write_crossing_type_rk_histogram_composites(
    *,
    homotypic_results: list[dict[str, Any]],
    heterotypic_results: list[dict[str, Any]],
    cutoffs: dict[str, float],
    ...
    """Write the homotypic and heterotypic three-panel histogram figures.

    The same global bin edges used for the six existing individual figures
    are recalculated from all retained R_K values, ensuring direct comparison.
    """
    homotypic, heterotypic = _collect_crossing_type_rk_values(
        homotypic_results=homotypic_results,
        heterotypic_results=heterotypic_results,
    ...
    return [
        _write_homotypic_rk_histogram_composite(
            homotypic=homotypic,
            bins=bins,
            output_root=output_root,
            rtotal_scale=rtotal_scale,
        ...
        _write_heterotypic_rk_histogram_composite(
...
def _statistics_row_plot_label(row: dict[str, Any]) -> str:
    ...
    if distribution in metric_symbol:
        return f"{crossing_type} " rf"$({metric_symbol[distribution]})$"
    ...
    if distribution == "Combined":
        ...
        return f"{crossing_type} " rf"${combined_symbols[crossing_type]}$"

    return f"{crossing_type} {distribution}"


RK_MEAN_STD_ANNOTATIONS: dict[tuple[str, str], dict[str, Any]] = {
    ("M-M", r"$R_{K_{\parallel}}$"): {"dx": -3.0, "dy": -3.0, "ha": "left", "va": "top", "arrow": False, "arrow_udlr": "down"},
    ("D-D", r"$R_{K_{\perp}}$"): {"dx": 15.0, "dy": -4.0, "ha": "left", "va": "top", "arrow": True, "arrow_udlr": "down"},
    ("Q-Q", r"$R_{K_Q}$"): {"dx": 15.0, "dy": 0.0, "ha": "left", "va": "bottom", "arrow": True, "arrow_udlr": "up"},
    ("M-D", r"$R_{K_{\perp}}$"): {"dx": 5.0, "dy": 9.0, "ha": "right", "va": "top", "arrow": False, "arrow_udlr": "up"},
    ("M-D", r"$R_{K_{\parallel}}$"): {"dx": 4.0, "dy": 4.0, "ha": "left", "va": "bottom", "arrow": False, "arrow_udlr": "up"},
    ("M-D", "Combined"): {"dx": 4.0, "dy": 4.0, "ha": "left", "va": "bottom", "arrow": False, "arrow_udlr": "up"},
    ("M-Q", r"$R_{K_Q}$"): {"dx": 5.0, "dy": 5.0, "ha": "right", "va": "bottom", "arrow": False, "arrow_udlr": "up"},
    ("M-Q", r"$R_{K_{\parallel}}$"): {"dx": 7.0, "dy": 2.0, "ha": "left", "va": "bottom", "arrow": False, "arrow_udlr": "up"},
    ("M-Q", "Combined"): {"dx": -12.0, "dy": 6.0, "ha": "left", "va": "bottom", "arrow": False, "arrow_udlr": "up"},
    ("D-Q", r"$R_{K_{\perp}}$"): {"dx": 5.0, "dy": 5.0, "ha": "left", "va": "bottom", "arrow": False, "arrow_udlr": "up"},
    ("D-Q", r"$R_{K_Q}$"): {"dx": -4.0, "dy": -12.0, "ha": "left", "va": "bottom", "arrow": False, "arrow_udlr": "up"},
    ("D-Q", "Combined"): {"dx": 8.0, "dy": 0.0, "ha": "left", "va": "bottom", "arrow": False, "arrow_udlr": "up"},
}


def _statistics_label_offset(row: dict[str, Any]) -> dict[str, Any]:
    key = (str(row["crossing_type"]), str(row["distribution"]))
    return RK_MEAN_STD_ANNOTATIONS.get(
        key,
        {"dx": 4.0, "dy": 4.0, "ha": "left", "va": "bottom", "arrow": False, "arrow_udlr": "up"},
    )


def _annotation_arrowprops(*, colour: str, arrow_udlr: str) -> dict[str, Any]:
    connection_style = {
        "up": "arc3,rad=0.08",
        "down": "arc3,rad=-0.08",
        "left": "arc3,rad=0.12",
        "right": "arc3,rad=-0.12",
    }.get(arrow_udlr, "arc3,rad=0.0")
    return {
        "arrowstyle": "->",
        "color": colour,
        "linewidth": 0.65,
...
def _write_rk_mean_vs_standard_deviation_plot(
    *,
    ...
    for row in valid_rows:
        ...
        annotation = _statistics_label_offset(row)
        arrowprops = (
            _annotation_arrowprops(
                colour=colour,
                arrow_udlr=str(annotation["arrow_udlr"]),
            )
            if bool(annotation["arrow"])
            else None
        )
        ...
        ax.annotate(
            ...
            xytext=(annotation["dx"], annotation["dy"]),
    ...
    ax.plot(
        ...
        linestyle="none",
        marker="o",
        markersize=0.82 * DEFAULT_MARKER_SIZE,
        markerfacecolor="none",
...
# -----------------------------------------------------------------------------
# Crossing-type R_K box-and-whisker plot
# -----------------------------------------------------------------------------

def _rk_box_plot_populations(
    *,
    homotypic_results: list[dict[str, Any]],
    heterotypic_results: list[dict[str, Any]],
    cutoffs: dict[str, float],
) -> tuple[list[str], list[list[float]], list[bool]]:
    """Return the nine requested R_K distributions for box plotting."""
    homotypic, heterotypic = _collect_crossing_type_rk_values(
        homotypic_results=homotypic_results,
        heterotypic_results=heterotypic_results,
        cutoffs=cutoffs,
    ...
    populations = [
        ...
        homotypic[1],
        homotypic[2],
        heterotypic["monopole_dipole"]["K_parallel"],
        heterotypic["monopole_dipole"]["K_perp"],
        heterotypic["monopole_quadrupole"]["K_parallel"],
        heterotypic["monopole_quadrupole"]["K_Q"],
        heterotypic["dipole_quadrupole"]["K_perp"],
        heterotypic["dipole_quadrupole"]["K_Q"],
    ]

    finite_populations: list[list[float]] = []
    for label, values in zip(labels, populations):
        finite_values = [
            float(value)
            for value in values
        ...
        if not finite_values:
            raise ValueError(
                f"No positive finite R_K values are available for {label}."
            )
        finite_populations.append(finite_values)

    is_homotypic = [True, True, True, False, False, False, False, False, False]
    return labels, finite_populations, is_homotypic


def _write_crossing_type_rk_box_plot(
    *,
    homotypic_results: list[dict[str, Any]],
    heterotypic_results: list[dict[str, Any]],
    cutoffs: dict[str, float],
    ...
    """Write a nine-distribution box-and-whisker plot of R_K.

    Boxes show the interquartile range, the line inside each box is the
    median, whiskers extend to the furthest non-outlier values within
    1.5 times the interquartile range, and points beyond the whiskers are
    shown individually as outliers.
    """
    if y_scale not in {"linear", "log"}:
        raise ValueError("RK_BOX_PLOT_SCALE must be 'linear' or 'log'.")

    labels, populations, is_homotypic = _rk_box_plot_populations(
        homotypic_results=homotypic_results,
        heterotypic_results=heterotypic_results,
        cutoffs=cutoffs,
    )
    ...
    for box, homotypic_box in zip(boxplot["boxes"], is_homotypic):
        colour = HOMOTYPIC_COLOUR if homotypic_box else HETEROTYPIC_COLOUR
        box.set_facecolor(colour)
        box.set_edgecolor(colour)
        box.set_alpha(0.28)
        box.set_linewidth(1.0)

    for distribution_index, homotypic_box in enumerate(is_homotypic):
        colour = HOMOTYPIC_COLOUR if homotypic_box else HETEROTYPIC_COLOUR
        for artist_index in (2 * distribution_index, 2 * distribution_index + 1):
            boxplot["whiskers"][artist_index].set_color(colour)
            boxplot["caps"][artist_index].set_color(colour)
        boxplot["medians"][distribution_index].set_color("black")
        boxplot["fliers"][distribution_index].set_markeredgecolor(colour)

    ax.set_yscale(y_scale)
    ax.set_ylabel(r"$R_K$", fontsize=AXIS_LABEL_FONTSIZE)
    ax.axhline(
        1.0,
        color=REFERENCE_COLOUR,
        linestyle="--",
    ...
    for tick_label in ax.get_xticklabels():
        ...
        tick_label.set_fontsize(7.0)

    ax.plot([], [], linestyle="none", marker="s", markersize=4.2,
            markerfacecolor="none", markeredgecolor=HOMOTYPIC_COLOUR,
            markeredgewidth=0.9, label="Homotypic")
    ax.plot([], [], linestyle="none", marker="s", markersize=4.2,
            markerfacecolor="none", markeredgecolor=HETEROTYPIC_COLOUR,
            markeredgewidth=0.9, label="Heterotypic")
    ...
    output_file = (
        output_root
        / RK_HISTOGRAM_SUBDIRECTORY
        / f"crossing_type_RK_box_and_whisker_{y_scale}.png"
    )
    output_file.parent.mkdir(parents=True, exist_ok=True)
    fig.savefig(output_file, bbox_inches="tight", dpi=dpi, format="png")
    plt.close(fig)
...
def write_rtotal_plots(
    *,
    ...
    rk_box_plot_scale: str = RK_BOX_PLOT_SCALE,
    ...
    written = [
        ...
        _write_combined_rtotal_overall_and_zoom_log_plot(
            combined_groups=combined_groups,
            cutoffs=cutoffs,
            output_root=output_root,
            ell_min=rtotal_zoom_ell_min,
        ...
        _write_ell_and_rtotal_histogram_two_panel(
            homotypic_rtotal=homotypic_rtotal,
            heterotypic_rtotal=heterotypic_rtotal,
            homotypic_ell=homotypic_ell,
            heterotypic_ell=heterotypic_ell,
            output_root=output_root,
            ell_n_bins=ell_histogram_n_bins,
            rtotal_n_bins=rtotal_histogram_n_bins,
            dpi=dpi,
        ),
    ]
    ...
    written.extend(
        _write_crossing_type_rk_histogram_composites(
            homotypic_results=homotypic_results,
            heterotypic_results=heterotypic_results,
            cutoffs=cutoffs,
            output_root=output_root,
    ...
    written.append(
        _write_crossing_type_rk_box_plot(
            homotypic_results=homotypic_results,
            heterotypic_results=heterotypic_results,
            cutoffs=cutoffs,
            output_root=output_root,
            y_scale=rk_box_plot_scale.lower(),
            dpi=dpi,
        )
    )
...
if __name__ == "__main__":
    write_rtotal_plots(
        ...
        rk_box_plot_scale=RK_BOX_PLOT_SCALE,
