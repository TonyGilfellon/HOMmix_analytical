...
figures used in the Hessian/Taylor workflow:
    ...
    k_perp     = (1/(4 U)) * |(c/omega) (c1, s1)|^2

    K_Q_raw    = (2 c/omega) * sqrt(|c2|^2 + |s2|^2)
    K_Q        = K_Q_raw/(sqrt(4 U) L) * 1e-12  [V/pC/m^3]
...
def figures_of_merit_from_rf_multipoles(
    ...
    *,
    ...
    if np.isfinite(U_J) and U_J > 0.0:
        denom = 4.0 * U_J
        kpar = abs(c0) ** 2 / denom
        kperp = (abs(Vperp_x_raw) ** 2 + abs(Vperp_y_raw) ** 2) / denom
        scale_K = 1.0 / np.sqrt(4.0 * U_J) / float(length_m) if length_m > 0 else float("nan")

        out.update({
            "loss_like_V_per_C": float(kpar),
            "loss_like_V_per_pC": float(kpar * PC),
            "loss_like_V_per_C_per_m": float(kpar / length_m) if length_m > 0 else float("nan"),
            "loss_like_V_per_pC_per_m": float(kpar * PC / length_m) if length_m > 0 else float("nan"),
            "kick_magnitude_V_per_C_per_m2": float(kperp),
            "kick_magnitude_V_per_pC_per_m2": float(kperp * PC),
            "kick_x_V_per_C_per_m2": float(abs(Vperp_x_raw) ** 2 / denom),
            "kick_y_V_per_C_per_m2": float(abs(Vperp_y_raw) ** 2 / denom),
            "kick_x_V_per_pC_per_m2": float(abs(Vperp_x_raw) ** 2 / denom * PC),
            "kick_y_V_per_pC_per_m2": float(abs(Vperp_y_raw) ** 2 / denom * PC),
            "K_normal_V_per_C_per_m3": float(abs(K_normal_raw) * scale_K),
            "K_skew_V_per_C_per_m3": float(abs(K_skew_raw) * scale_K),
            "KQ_V_per_C_per_m3": float(KQ_raw * scale_K),
            "K_normal_V_per_pC_per_m3": float(abs(K_normal_raw) * scale_K * PC),
            "K_skew_V_per_pC_per_m3": float(abs(K_skew_raw) * scale_K * PC),
            "KQ_V_per_pC_per_m3": float(KQ_raw * scale_K * PC),
            "U_CST_normalisation_J": float(U_J),
            "U_CST_normalisation_note": (
                "RF multipole metrics: k_parallel=|c0|^2/(4U); "
                "k_perp=|(c/omega)(c1,s1)|^2/(4U); "
                "KQ=4(c/omega)sqrt(|c2|^2+|s2|^2)/sqrt(4U)/L, "
                "matching the Hessian/Taylor scalar sqrt((Kxx-Kyy)^2+4Kxy^2)."
            ),
        })
    return out
...
def compare_parent_and_mixed_figures(fields: dict[str, Any]) -> dict[str, Any]:
    metric_keys = {
        "k_parallel_V_per_pC": "loss_like_V_per_pC",
        "k_parallel_V_per_pC_per_m": "loss_like_V_per_pC_per_m",
        "k_perp_V_per_pC_per_m2": "kick_magnitude_V_per_pC_per_m2",
        "KQ_V_per_pC_per_m3": "KQ_V_per_pC_per_m3",
    }
    out: dict[str, Any] = {}
    ...
    *,
    ...
    out = {
        ...
        "units": {
            ...
            "c1_s1": "V/C/m",
            "c2_s2": "V/C/m^2",
            "k_parallel": "V/pC or V/pC/m as labelled",
            "k_perp": "V/pC/m^2",
            "KQ": "V/pC/m^3",
            "U": "J",
        },
...
def write_text_summary(result: dict[str, Any], filename: str | Path) -> None:
    ...
    for name in ("E1", "E2", "plus", "minus"):
        ...
        lines.append(f"  k_parallel              = {fom.get('loss_like_V_per_pC', float('nan')):.12e} V/pC")
        lines.append(f"  k_perp                  = {fom.get('kick_magnitude_V_per_pC_per_m2', float('nan')):.12e} V/pC/m^2")
        lines.append(f"  KQ                      = {fom.get('KQ_V_per_pC_per_m3', float('nan')):.12e} V/pC/m^3")
        lines.append("")
